...
export async function loadProductionSharingRolloutConfig(filePath) {
  let handle;
  try {
    if (!path.isAbsolute(filePath) || path.normalize(filePath) !== filePath) fail("sharing_rollout_config_path_invalid");
    handle = await open(filePath, constants.O_RDONLY | constants.O_NOFOLLOW);
    const info = await handle.stat();
    if (!info.isFile() || info.size < 2 || info.size > 32 * 1024 || (info.mode & 0o077) !== 0) {
      fail("sharing_rollout_config_file_unsafe");
    }
    ...
    if (raw.schemaVersion !== 1 || raw.environment !== "production") fail("sharing_rollout_config_invalid");
    if (raw.host.architecture !== "amd64") fail("sharing_rollout_architecture_invalid");
    const origin = httpsOrigin(raw.gateway.origin);
    if (raw.gateway.runtimeContract !== "hermes-serve-v1") fail("sharing_rollout_runtime_contract_invalid");
...
function fail(cause) {
  throw new OpsError("productionSharingRollout", cause, "production_sharing_rollout_config");
}
