...
2026-09-09 在邮箱登录灰度迁移至 schema 15 后，已完成新的全链路代次
`20260908T191154059Z-cecbfc922361`：87,632 字节生产密文由 Mac 接收，在一次性 PostgreSQL 18 中精确恢复，
使用 `main 787bdc917190` 的 Gateway 0.4.9 不可变镜像完成 schema 15 与账号事务 smoke，证据/状态回传后由
生产端写入 `activated.ack.json` 并原子激活。生产监控同步期待 schema 15 后，主机、磁盘和异机备份三项
立即复查全绿。
...
- 捕获前后都精确检查 PostgreSQL 18、配置指定的 schema（当前生产为 15）和服务 active；变化、版本不符、
  密文为空或超限均失败。
...
截至 2026-09-09，R5-E1 至 R5-E7A 的实现、手动闭环、生产/Mac 两端调度、自动捕获、异机恢复、状态激活
和监控均已完成。R5-F 邮箱登录灰度已迁移到 schema 15；迁移后的新代次已再次通过真实异机恢复、不可变
0.4.9 镜像账号 smoke、状态激活、ack 和新鲜度检查。首次 schema-15 捕获因 URL 文件权限为 `0440` 而在
dump 前失败关闭，改用既有 root-only `0600` 受保护源后成功；首次 Mac 恢复因精确目标镜像尚未加载而在
smoke 前失败关闭，加载 manifest 绑定的镜像后成功。两条失败路径均未发布伪证据或有效状态。Mac 小时级
LaunchAgent 已用新 operator 重载并幂等退出 0，HK 每日捕获与 15 分钟生产监控 timer 均保持 active。
