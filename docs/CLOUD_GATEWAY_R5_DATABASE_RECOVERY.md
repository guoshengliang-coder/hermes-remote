...
- 恢复目标必须是没有用户表的空数据库。解密输出通过管道直接进入 `pg_restore --single-transaction`，不在
  磁盘落地明文 dump。
- 账号 smoke 先核对本地镜像 ID，再用该内容寻址 ID 启动只读、无 capability、限内存/CPU/PID 的一次性
  容器。容器在事务中写入账号、Google identity、Android installation 并做关联读取，最后始终 rollback。
...
容器，构建当前提交的内容寻址 Gateway 镜像，并真实执行上述完整链路。测试账号、数据库密码、恢复密钥、
