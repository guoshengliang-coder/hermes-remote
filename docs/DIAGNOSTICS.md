...
- `stale local lead expired (HG-124), accepting over: …` → 本地领先是服务端永远不会有的行（一次失败或
  中断的发送留下的）。运行时闲置且安静超过 10 分钟、快照末行又严格新于本地末行时，对账接受这份快照、
  本地那行退场。HG-124 第二轮修复之前它会**永远**被拒（20260924_102646_68e7a7：三小时里六次全量拉取
  全被拒，只有手动刷新能救），同一修复也让 open() 重进不再被 keepLive 条款丢弃覆盖性快照 —— 若日志里
  还有「重进后旧内容、手动刷新才出新内容」而**没有**这行过期日志，说明走的是别的路径，按第 1 问继续。
