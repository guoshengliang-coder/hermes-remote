...
- **上一组对话胶囊**（决策 2026-09-02，设计稿
  ...
  - 点按：把该组用户气泡顶边对齐到列表顶部（倒序列表需用 item 高度换算偏移；可见时动画，
    ...
    **到边即停**（决策 2026-09-03）：最近一两组的提问下方内容不够，顶不到顶部时，列表停在底端即可，
    不再逐帧追赶；逐帧修正一律用绝对定位 `scrollToItem(index, offset)`，禁止相对 `scrollBy`
    （旧实现在这种情况下每帧在两个位置间弹跳 3 秒并吞掉触摸，真机复现于 2026-09-03）。
