...
### 5.9 检查更新页（决策 2026-09-01）
- 页面是**单一推荐更新**界面，不是可任选 APK 的版本仓库：只有 manifest 的
  `latestVersionCode` 可下载/安装；历史版本默认折叠且只读。恢复出的旧下载在离线时仍可
  继续，但联网发现更高 `latestVersionCode` 后必须阻止安装，只保留删除旧下载的出口。
- 进入页面即自动检查，同时独立恢复本地下载任务；断网或索引失败不得隐藏已下载、可安装
  的任务。保留「重新检查」作为显式恢复操作，不自动下载或自动安装。
- 主卡必须明确区分：正在检查、已是最新版、发现更新、排队/暂停/下载、校验、可安装、失败。
  第一阶段按钮写「下载更新」，签名与完整性校验通过后才写「安装更新」；安装仍由系统安装器
  要求用户确认。
- 下载任务可取消，失败可重试；暂停原因、下载错误和安全校验失败使用各自的双语错误码与
  恢复动作。技术原因仅在脱敏详情中显示。
- TalkBack 只播报阶段变化，不逐百分比播报；版本标题使用 heading 语义，所有按钮保持
  48dp 最小触达区。
