...
- **上一组对话胶囊**（决策 2026-09-02，设计稿
  ...
  - **分段胶囊**：视口所在组从最新一组往上数已是第 3 组或更早（`TURN_PILL_LIST_DEPTH = 2`）时，
    胶囊右侧展开列表段（1dp 分隔线 + `PromptListIcon`），点按打开「我的提问」；回到最近两组收起。
    规则无状态，只看当前位置。第一版**不加**上一组 / 下一组箭头。
- **我的提问**（决策 2026-09-02）：顶栏更多菜单第一项（导航在动作之前），打开底部弹层
  （§5.5 / §5.8 规范：左对齐标题「我的提问」、内容多高弹多高）。每行 = 提问首行（bodyLarge，
  ≤2 行）+ 时间（labelMedium，沿用时间分隔线格式）；当前所在组底色 surfaceContainerHigh、
  副行 primary「时间 · 当前位置」并带箭头抵线图标，打开时滚到该行；有「会话开始」段时首行
  为灰色「会话开始」、无时间。点行 = 关闭弹层并按胶囊同一规则跳到该组起点。
  英文文案 Your prompts / You are here / Start of chat。
