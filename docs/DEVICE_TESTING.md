...
**真机低于 `targetSdk` 那条 NOTE 不是本次改动的缺口。** 它是这台机器的固定属性，每跑必打，
只对一类改动成立：应用因为抬高 `targetSdk` 才启用的平台行为。视觉/布局/文案、通知、后台存活、
省电策略、权限弹窗——这些在手上的真机上都验得了，SDK 33/34 一样算数。照抄这句到交接说明里，
会让读的人以为你该验的没验；2026-09-19 就发生过一次（HG-65 那批改动里没有一样是 `targetSdk`
门控的，却被当成缺口报了出去）。脚本 2026-09-19 起把措辞从 `GAP:` 改成了 `NOTE:` 并写明适用范围。
...
| 厂商 | 确认流程 | 状态 |
|---|---|---|
| **vivo** V2166BA（Android 13 / SDK 33） | 一页「安全守护提示您 · 外部来源应用」。勾「已了解应用的风险检测结果」后再点「继续安装」。装完直接回到桌面、没有「完成」页。按钮在控件树里一直是 enabled，灰色只是画出来的，所以要以勾选框读到 `checked=true` 为准。**2026-09-21 起点「继续安装」后还会进 vivo 账号身份验证页**（`com.bbk.account/.activity.AccountVerifyActivity`，页面底部写「继续安装第三方应用需身份验证，点击右上角设置可关闭验证」）：要输账号密码，截图是空白（安全窗口）。按红线不碰，驱动报 `NEEDS_ATTENTION` 后交给主人输密码；关不关那个验证开关是主人的决定，改它属于改手机安全设置 | **已收录**配方 `vivo-external-source`（2026-09-11）。身份验证页**不收录、也不能收录**：它要密码 |
...
- **别人留下的安装确认页会挡住你的安装。** 2026-09-21 vivo 上停着一个约两小时前由已退出的插桩测试
  发起的 `com.hermes.remote.test` 确认页，后面的安装会话一直排在它后面、页面不出来。先用
  `dumpsys package` 看 Active install sessions（按 `sizeBytes` 认出是谁），确认发起者已经不在了，
  再按一次返回键关掉那一页 —— **不点里面的确认**。之后自己的会话可能不会自动重新弹页，
  停掉自己的 `adb install` 重装一次即可。插桩测试（`connectedAndroidTest`）会卸载重装 App，
  **两台手机的 App 数据（登录状态、通知权限）都会没掉**：那天两台手机的 `firstInstallTime`
  都变成了同一时刻。跑之前先告诉主人。
...
- **要验上游新的 server→client 请求协议**（审批 / 追问走 `srq-…` 请求而不是事件），用
  `HR_MOCK_SERVER_REQUESTS=1 HR_MOCK_STRICT_PARAMS=1`：每次运行先发一个审批、再发一个轮换形式的
  追问（单选 → 多选 → 三题批次），多余参数按 17b5df02 回 4000。每个请求默认 60 秒超时；让 AI 用
  「每步先截图、按控件树点」的方式驱动时 60 秒不够答完一个三题批次，调大
  `HR_MOCK_REQUEST_TIMEOUT_MS=300000`。
- **在真实 Hermes 上几乎触发不了审批卡片。** 主人的 `approvals.mode` 是 `smart`，由守护模型判断
  命令是否危险：2026-09-21 在 `/tmp` 下的 `rm -rf`、`chmod -R 777`、`base64 -d | sh` 都被直接放行。
  所以审批的允许 / 拒绝 / 通知栏 / 恢复要在 mock 上验；追问（clarify）在真实 Hermes 上可以稳定触发
  （「用 clarify 工具问我选 A 还是 B」）。
- **连真实 Hermes 做冒烟时，本地 gateway 必须 `HOST=127.0.0.1`，token 用随机值。** gateway 默认
  监听 `0.0.0.0`、开发 token 是公开的 `dev-app-token` —— 对 mock 无所谓，对真实 Hermes 等于把
  "让 agent 在 Mac 上执行命令"开放给整个局域网。
...
7. **连不上 Firebase（HG-102）。** 在 VPN 客户端里把 Hermes GO 排除在代理之外，再点一次「重试」
   （或清除应用数据后重新登录）。「实时推送」行显示「正在注册…」不超过 30 秒，然后变为
   `HR-NOTIF-002` + 「重试」，诊断日志 `[push]` 行带 `stage=fcm_token`；把 App 放回 VPN 后点
   「重试」应变为「已启用」。以前这一步会永远停在「正在注册…」。
8. **推送唤醒时的桌面角标（HG-103，荣耀/华为）。** 先正常打开一次 App，让会话列表加载一次
   （已知会话集合就是这一次写盘的）。然后划掉 App，从 Mac 发起一个会产出回答的任务。推送到达、
   通知弹出时，桌面图标角标应**同时** +1，不必等打开 App。打开 App、进入该会话读完再退回桌面，
   角标应回到原值，不应先变成 +1 或停在 1。
...
### HG-139：冷启动和前台断网的真机验收

1. 保留已配置、可正常连接的服务地址。关闭移动数据，用 Wi-Fi 正常进入 App；随后划掉 App。
   关闭 Wi-Fi 后冷启动，在自动重试文案出现时恢复 Wi-Fi。确认无需手点重试便进入原页面，
   且没有先闪出错误页。重复一次，让网络持续断开直到重试耗尽，确认最终错误含 `HR-*` 码和可操作的重试按钮。
2. 在已进入会话页时短暂断网并恢复，确认聊天内容始终留在屏幕上；持续断网时顶部连接提示出现，
   恢复后自动消失。切换 Wi-Fi / 移动数据各测一次，记录设备型号、系统版本和出现文案的时间。
3. 若要验证「找不到服务地址」等具体归类，使用可控测试网络制造该失败，记录本机诊断日志中的各次
   状态探测与公开健康检测结果。不能仅凭浏览器打开网页成功就宣称 App 的域名解析必然成功。
   测完恢复网络设置；这部分只能在真机或受控网络上验证，JVM 测试仅证明归类和文案映射。
