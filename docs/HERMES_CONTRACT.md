...
| `https://hermes-agent.nousresearch.com/install.sh` (source `scripts/install.sh`), added 2026-09-22 | install-when-missing downloads it over HTTPS (final URL on that host or under `raw.githubusercontent.com/NousResearch/hermes-agent/`), at most 2 MiB, starting with `#!`; upstream publishes no checksum or signature (`docs/MANAGED_HERMES_STRATEGY.md`, "Installer trust decision") | a moved URL or a redirect off the origin is `HR-MIGRATE-015`/`017`; if upstream starts publishing a checksum, verify it |
...
8l. **Installer protocol** (install-when-missing, section 8). Read the new commit's
    ...
    `$HERMES_HOME/hermes-agent`; no macOS path added `sudo`; and upstream still publishes no checksum
