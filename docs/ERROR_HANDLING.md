...
| `HR-CRON-003` | An action on a scheduled job was refused or could not be sent: `POST /api/cron/jobs/{id}/{pause\|resume\|trigger}` returned a non-2xx the client cannot map to a stable code, or the request never completed. Distinct from `HR-CRON-002`, which is about a run that ALREADY happened — this one is about the tap the user just made. When the server does send a stable code, that code is shown instead of this one | 操作没有成功，请查看详情后重试。 | The action didn't go through. Review the details and retry. | Depends |
