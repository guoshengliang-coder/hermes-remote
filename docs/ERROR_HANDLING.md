...
| `HR-WEB-001` | A browser (Web app) session asked the Gateway for a device route outside the browser allowlist (`gateway/src/account/web-device-access.ts`), or sent a JSON-RPC method outside the browser tunnel's method allowlist (`gateway/src/account/web-rpc-filter.ts`): configuration, secrets, scheduled tasks, skills, messaging, slash commands, Gateway restart and every other administration route stay with the Android and Desktop apps. REST answers HTTP 403; the WebSocket answers the request id in-band with JSON-RPC error `4403` and `data.code = "HR-WEB-001"`. Nothing reaches the Mac in either case | 网页版不支持此功能，请使用 Android 应用。 | This feature isn't available in the Hermes GO web app. Use the Android app instead. | No |
...
| `HR-MIGRATE-001` | Managed installation preflight, signed package/component preparation, or pre-mutation cleanup failed before service mutation | 暂时无法升级连接，现有连接未被修改。 | The connection can't be upgraded yet. The existing connection was not changed. | Yes |
...
| `HR-MIGRATE-003` | The managed Hermes/Connector candidate failed authentication or health and automatic rollback restored the prior connection | 新连接验证失败，已恢复原来的连接。 | The new connection failed validation, so the original connection was restored. | Yes |
| `HR-MIGRATE-004` | Automatic rollback could not prove a known-good Hermes/Connector topology and stopped further mutation to avoid a retry loop. The diagnostic names the operation, the error that started the rollback and the error that stopped it (`rollbackFailed operation=managed-upgrade cause=… recovery=…`) | 自动恢复未完成，系统已停止继续变更。请按诊断步骤检查 Hermes 与 Connector。 | Automatic recovery did not finish, so further changes were stopped. Follow the diagnostic steps to inspect Hermes and Connector. | No (manual recovery) |
...
| `HR-MIGRATE-006` | The `state.db` on this Mac carries columns the managed Hermes was not built to read. The managed copy is pinned to one upstream commit while the owner's own Hermes rolls forward, and they share one database by design (`docs/MANAGED_HERMES_STRATEGY.md`), so this is drift, not corruption. Detected by comparing the live columns against the baseline recorded in the component's `BUILD-IDENTITY.json` — **not** against `schema_version`, which upstream left at 30 while adding `display_identity` and `display_order` on 2026-09-19, the very change that caused the incident. Nothing is broken yet: a read that touches an unknown column may return an unusable response, and the symptom surfaces on the phone while the Mac looks healthy. Columns already examined and found harmless are subtracted (`DesktopManagedSchemaAcknowledgement.known`), and only on their recorded terms — the neutralising patch must be in that release's `BUILD-IDENTITY.json` and the column's type must be unchanged — so that the notice is not permanently lit and is therefore still read the one time it is new | 这台 Mac 的数据库比托管 Hermes 新，请升级托管副本。 | This Mac's database is newer than the managed Hermes. Update the managed copy. | No (update the managed Hermes) |
| `HR-MIGRATE-007` | A startup repair of committed managed-Hermes token storage or LaunchAgent search path failed. The active services remain available and a separately safe managed upgrade remains actionable; this is not evidence of a duplicate Connector. The diagnostic carries the repair stage, while raw exceptions remain behind Details | 当前服务仍可使用，后台补救未完成；请查看详情后重试。 | The current services remain available, but a background repair did not finish. Review the details and retry. | Yes |
| `HR-MIGRATE-008` | This Mac already has Hermes, but not in the standard layout Hermes GO can run on the owner's behalf (`HERMES_HOME=~/.hermes`, checkout `~/.hermes/hermes-agent`, venv entrypoint, single default profile, version at or above `DesktopLocalHermesDetector.minimumVersion`). Raised by local runtime mode (`docs/DESKTOP_PHASE0.md`, "Local Hermes runtime") for profiles, a custom `HERMES_HOME`, a second install or entrypoint outside the checkout, Hermes data without the checkout, an incomplete/unsafe/too-old checkout, or an unreadable `HEAD`/version. Nothing is switched, and a fresh managed install is refused rather than placing a second copy of Hermes beside the owner's; an existing installation keeps running whatever it runs. The reason is carried behind Details as `reason=<name>`; paths under `/Users/<name>` are redacted to `/Users/<user>` (`SecretRedactor`). Not shown while an upstream update holds its lock. Also raised as `reason=dependenciesInconsistent` when the venv does not match the checkout before Desktop would start new code: a version mismatch that persists beyond ten minutes, or zero or several `hermes_agent-*.dist-info` (shown at once) | 这台 Mac 已有 Hermes，但安装方式不是 Hermes GO 能直接使用的标准形式；为避免出现第二份 Hermes，未做任何更改。请查看详情。 | This Mac already has Hermes, but not in the standard layout Hermes GO can use. To avoid a second copy of Hermes, nothing was changed. Review the details. | No (change the Mac's Hermes setup) |
| `HR-MIGRATE-009` | Desktop could not switch its Hermes service to this Mac's own Hermes, restart it onto updated code, reload an agent launchd was running with different arguments, or return it to the bundled copy — or the owner's Hermes keeps stopping after Desktop restarts it (three Desktop starts within 30 minutes pause further restarts until the window passes). Every failure path leaves the Hermes job loaded: a failed switch restores and restarts the bundled agent; a failed return to bundled restores and restarts the local agent when the owner's Hermes is still usable (the setting-off rollback) and otherwise keeps the bundled agent loaded; a failed restart onto new local code restores the agent file and loads it again, and is **not** rolled back to the bundled copy, because that would put pinned code back on a database the new code may already have migrated. Desktop retries after a five-minute back-off; another operation holding the migration lease is not an error and is retried on the next refresh. The diagnostic carries the stage and the underlying error; when the restore failed too it names the operation, the original error and the restore error (`rollbackFailed operation=… cause=… recovery=…`), with home-directory names redacted. If the job could not be loaded at all the code is `HR-MIGRATE-013` instead | Hermes GO 未能启动或切换本机 Hermes；能恢复的已恢复为原来的服务。请查看详情，稍后会自动重试。 | Hermes GO couldn't start or switch to this Mac's Hermes. Whatever could be restored was restored. Review the details; it will retry automatically. | Yes (automatic, after back-off) |
| `HR-MIGRATE-010` | Local runtime mode is configured, the owner's Hermes is gone, and no usable bundled agent is kept to return to — the kept agent is missing, malformed, stores its token inline, or names a program or Python runtime that no longer exists (for example after component garbage collection). Nothing is changed; retrying cannot help | 这台 Mac 上的 Hermes 已被移除，且没有可恢复的内置 Hermes。请重新安装 Hermes 或 Hermes GO。 | This Mac's Hermes was removed and there is no built-in Hermes to return to. Reinstall Hermes or Hermes GO. | No (reinstall Hermes or Hermes GO) |
| `HR-MIGRATE-011` | Automatic Hermes runtime changes paused after repeated failures (`DesktopHermesRuntimePlanner.maximumAttempts`, two): either switching to this Mac's own Hermes failed on the same commit (typically it never printed `HERMES_BACKEND_READY` within the readiness window; each attempt costs the phone its Hermes for minutes), or, with the setting off, returning to the kept bundled agent failed and the owner's Hermes was restored each time. The current service stays as it is. A switch resumes when the owner's commit changes or the setting is turned off and on; a rollback resumes when the kept bundled agent changes or the setting is turned on | 多次尝试切换 Hermes 均未成功，已暂停自动切换，当前服务保持不变。更新 Hermes 或重新开关本机 Hermes 设置后会再试。 | Switching Hermes failed several times, so automatic switching is paused and the current service is unchanged. It will try again after Hermes is updated or the setting is toggled. | No (update Hermes or toggle the setting) |
| `HR-MIGRATE-012` | The local-runtime setting is off, but no usable bundled agent is kept to return to while this Mac's own Hermes is intact and keeps running. Distinct from `HR-MIGRATE-010`: nothing was removed, and the Mac is not without Hermes | 本机 Hermes 设置已关闭，但没有可用的内置 Hermes 可恢复；Hermes GO 会继续使用这台 Mac 自己的 Hermes。重新安装 Hermes GO 后可恢复。 | The setting is off, but there's no usable built-in Hermes to return to, so Hermes GO keeps using this Mac's own Hermes. Reinstall Hermes GO to restore it. | No (reinstall Hermes GO) |
| `HR-MIGRATE-013` | The managed Hermes job (`com.hermesgo.hermes-server`) is not running — Desktop stopped it during a runtime switch, restart, restore or reload, or found it not loaded — the operation failed, and launchd then refused every attempt to load the job again (three bootstrap attempts with 2 s and 4 s back-off, because launchd can refuse transiently right after a bootout). Nothing serves the phone until the job is loaded; distinct from `HR-MIGRATE-009`, whose failures leave a job loaded. Desktop retries after the five-minute runtime back-off — a bundled job that is not loaded is loaded again by every reconciliation, with the setting on or off. The diagnostic names the operation, the original error and each refusal, with launchd's own standard error in `Managed/logs/desktop-runtime.log`; home-directory names are redacted | Hermes 服务没有在运行，Hermes GO 多次尝试载入均未成功，手机暂时无法使用这台 Mac。稍后会自动重试；请查看详情。 | The Hermes service isn't running and Hermes GO couldn't load it, so the phone can't reach this Mac for now. It will retry automatically; review the details. | Yes (automatic, after back-off) |
| `HR-MIGRATE-014` | The managed Hermes job is not loaded (Desktop found it so; it stopped nothing), and one probe shows `127.0.0.1:9119` already accepting connections — or not provably free — so another process holds the port (for example the owner's own Hermes or dashboard). Desktop does **not** load the job: it would only crash-loop on `EADDRINUSE`, opening the shared `state.db` on every start. There is no 75-attempt stop wait and no reload. Desktop retries after the five-minute runtime back-off; the fix is to stop whatever holds the port | Hermes 服务没有在运行，但端口 9119 已被这台 Mac 上的其他程序占用，Hermes GO 未启动它以免冲突。请关闭占用端口的程序；稍后会自动重试。 | The Hermes service isn't running, but another program on this Mac is using port 9119, so Hermes GO didn't start it to avoid a conflict. Close that program; it will retry automatically. | Yes (automatic, after back-off) |
...
| `HR-OPS-022` | Production multi-device route/flag installation, restart, smoke verification, or restoration of exact single-device state did not complete | 生产多终端灰度未完成，已阻止启用或恢复为单终端状态。请检查灰度阶段后重试。 | The production multi-device rollout did not complete. Enablement was blocked or single-device mode was restored. Inspect the rollout stage and retry. | Yes (inspect the protected multi-device journal and retry only after confirming the original single-device environment and routes are restored) |
    | `HR-OPS-023` | Production email-identity and Web account-center route/flag installation, restart, security smoke verification, or restoration of exact multi-device state did not complete | 生产身份与 Web 账号中心灰度未完成，已阻止启用或恢复为多终端状态。请检查灰度阶段后重试。 | The production identity and Web account-center rollout did not complete. Enablement was blocked or multi-device mode was restored. Inspect the rollout stage and retry. | Yes (inspect the protected identity-Web journal and retry only after confirming the original multi-device environment and routes are restored) |
    | `HR-OPS-024` | Production whole-device sharing route/flag installation, restart, guard verification, or restoration of exact identity-Web state did not complete | 生产整机共享灰度未完成，已阻止启用或恢复为身份与 Web 账号中心状态。请检查灰度阶段后重试。 | The production whole-device sharing rollout did not complete. Enablement was blocked or the identity and Web account-center state was restored. Inspect the rollout stage and retry. | Yes (inspect the protected sharing journal and retry only after confirming the original identity-Web environment and routes are restored) |
    | `HR-OPS-025` | Production Desktop component-manifest validation, capability enablement, restart, smoke verification, or restoration of the previous capability state did not complete | 生产 Desktop 组件安装灰度未完成，已阻止启用或恢复为原组件能力状态。请检查灰度阶段后重试。 | The production Desktop component rollout did not complete. Enablement was blocked or the previous component capability was restored. Inspect the rollout stage and retry. | Yes (inspect the protected component-rollout journal and retry only after confirming the previous Gateway environment and capability are restored) |
...
| `HR-SESS-013` | Another client already owns the conversation. A managed Hermes with the read-only `session.access` projection reports this on open/reconnect and the composer becomes read-only before a send; older Hermes versions still surface it when `prompt.submit` is refused with 4090. The conversation still exists and the conflict clears as soon as the other side finishes | 该会话正在另一个客户端上运行，请在那边结束后重试。 | This conversation is running on another client. Finish it there, then retry. | Yes |
...
| `HR-SESS-015` | A refused send restored after the app restarted, whose staged attachments did not survive. Only the text is persisted (`data/repository/UnsentStore.kt`, the same ruling drafts get: attachments are in-memory bytes and persisting them is a different feature), so replaying the send would deliver less than the user meant. The bubble keeps the words and names this instead of the generic `HR-SESS-007`, and the tap is withheld — a retry that silently drops the images is the kind of offer HG-29 ruled out | 附件已丢失，请重新选择附件后发送。 | The attachments are gone. Pick them again and send. | **No** |
| `HR-SESS-016` | The Mac's Hermes could not find its PDF rendering dependency, so a PDF attachment was refused (`pdf.attach` 5028). The binary (`pdftoppm`, from poppler-utils) may well be installed — upstream's wording means "not on my PATH", and the managed Hermes runs as a launchd agent whose PATH is the bare `/usr/bin:/bin:/usr/sbin:/sbin` (HG-58). Either way nothing about the conversation is wrong and retrying changes nothing until the Mac side is fixed, so the tap is withheld the way HG-29 requires | Mac 上的 Hermes 找不到 PDF 渲染依赖，无法附加 PDF。 | Hermes on the Mac can't find its PDF rendering dependency, so the PDF can't be attached. | **No** |
| `HR-SESS-017` | The Mac answered, but its answer was larger than the relay carries one frame in, so the Connector dropped it and synthesized a JSON-RPC `-32001` in its place (`connector/src/oversized-frame.ts`). What is oversized is the conversation, not the request: Hermes stores an attachment as a reference but re-inlines it as base64 on every read, so one 37-page PDF makes a `session.resume` answer 12.59 MiB and the whole transcript 26.3 MiB (HG-65). The next attempt is the same bytes and the total only grows, so the tap is withheld the way HG-29 requires. The conversation is still readable — history goes over the chunked REST path, which the frame ceiling does not apply to | 这个会话的内容太大，Mac 无法把它传过来，请开新会话继续。 | This conversation is too large for the Mac to send. Start a new one to continue. | **No** |
| `HR-CLARIFY-001` | Clarify answer arrived after the request expired server-side. f159e581: `clarify.respond` answers `expired`. Newer Hermes: `clarify.lock` / `request.answer` answer `expired` — the request timed out, the run was interrupted, or another surface answered it first (that last case sends no `request.cancel`, so only the answer can reveal it) | 这个提问已失效，agent 没有收到这次回答，请在输入框直接说明你的选择。 | The clarify question expired before the answer arrived; tell the agent your choice in the composer. | No |
...
| `HR-APPROVAL-002` | The conversation is waiting on an approval whose request did not survive the app restart. An approval carries no id and `approval.respond` addresses only the session, so a card rebuilt from a local snapshot could approve a command the user never saw; it is deliberately not restored. Not shown when a newer Hermes hands the open approval back in `session.resume`'s `open_requests`, because then the card is restored by Hermes itself | 这条会话在等你确认，但那次审批请求没能在 App 重启后保留下来，这里无法再批准它，请在输入框直接说明你的决定。 | This conversation is waiting for your approval, but the request didn't survive the app restart, so it can't be approved from here. Tell the agent your decision in the composer. | No |
| `HR-APPROVAL-003` | An approve/deny for a server→client approval request (newer Hermes) reached a request that was no longer open: `request.answer` answered `expired` because it timed out, the run was stopped, or another surface (desktop, another phone) answered first. Unlike `HR-APPROVAL-001` this is reported by Hermes, not inferred, and it does not claim who decided | 这次审批没有送达：这条审批已不再等待回答（可能已超时、运行已停止，或已在其他设备上处理）。如仍需要，请在输入框重新说明。 | This approval didn't reach the agent: the request was no longer waiting (it may have timed out, the run stopped, or it was answered on another device). If you still want it, say so in the composer. | No |
...
| `HR-SYNC-003` | The Mac's Hermes answered the transcript request with a 5xx. The conversation and the connection are both fine; the failure is inside Hermes, and the relay forwarded it faithfully (its own log reads `outcome=streamed`). Retrying repeats it, so the tap is withheld and the copy points at the Mac. Registered after 2026-09-19, when a schema drift made every affected transcript 500 and the phone said only `HR-RPC-001` — two hours went into a cause the status code would have named | Mac 上的 Hermes 返回了错误，请检查 Mac 端。 | Hermes on the Mac returned an error. Check the Mac. | **No** |
| `HR-SYNC-004` | The transcript arrived but could not be parsed — a shape this build does not understand. Distinct from a 5xx (the Mac is fine) and from a dropped connection (the bytes arrived). Retrying cannot help, because the same bytes parse the same way; the app has to be updated. The 2026-09-18 content-block arrays were this, and they read as a generic failure until `MessageContentSerializer` landed | 无法解析会话内容，请更新 App。 | This conversation could not be read. Update the app. | **No** |
...
| `HR-CRON-003` | An action on a scheduled job was refused or could not be sent: `POST /api/cron/jobs/{id}/{pause\|resume\|trigger}` returned a non-2xx the client cannot map to a stable code, or the request never completed. Distinct from `HR-CRON-002`, which is about a run that ALREADY happened — this one is about the tap the user just made. When the server does send a stable code, that code is shown instead of this one. **A timeout or trigger HTTP 409 is not by itself this code**: 「立即运行」 runs the job synchronously upstream (`docs/HERMES_CONTRACT.md` §7c), so a job slower than the REST timeout times out while running, while a concurrent scheduler can acquire `fire_claim` before the tap. The client re-reads the job and, when `fire_claim` is held or `last_run_at` has moved, reports 「已触发，正在后台运行」 instead — this code is left for an uncertain response with no run behind it | 操作没有成功，请查看详情后重试。 | The action didn't go through. Review the details and retry. | Depends |
