...
| `HR-MIGRATE-013` | Desktop stopped the managed Hermes job (`com.hermesgo.hermes-server`) during a runtime switch, restart, restore, reload or load, the operation failed, and launchd then refused every attempt to load the job again (three bootstrap attempts with 2 s and 4 s back-off, because launchd can refuse transiently right after a bootout). Nothing serves the phone until the job is loaded; distinct from `HR-MIGRATE-009`, whose failures leave a job loaded. Desktop retries after the five-minute runtime back-off — a bundled job that is not loaded is loaded again by every reconciliation, with the setting on or off. The diagnostic names the operation, the original error and each refusal, with launchd's own standard error in `Managed/logs/desktop-runtime.log`; home-directory names are redacted | Hermes GO 重启 Hermes 服务后未能重新载入，手机暂时无法使用这台 Mac。稍后会自动重试；请查看详情。 | Hermes GO restarted the Hermes service but couldn't load it again, so the phone can't reach this Mac for now. It will retry automatically; review the details. | Yes (automatic, after back-off) |
