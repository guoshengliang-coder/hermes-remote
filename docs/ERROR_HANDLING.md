...
| `HR-CLARIFY-001` | Clarify answer arrived after the request expired server-side (on a newer Hermes only a batch `clarify.lock` can report this; a single answer gets no reply, and an expired request is torn down by `request.cancel` instead) | 这个提问已失效，agent 没有收到这次回答，请在输入框直接说明你的选择。 | The clarify question expired before the answer arrived; tell the agent your choice in the composer. | No |
