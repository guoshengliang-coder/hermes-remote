...
| `HR-MIGRATE-006` | The `state.db` on this Mac carries columns the managed Hermes was not built to read. The managed copy is pinned to one upstream commit while the owner's own Hermes rolls forward, and they share one database by design (`docs/MANAGED_HERMES_STRATEGY.md`), so this is drift, not corruption. Detected by comparing the live columns against the baseline recorded in the component's `BUILD-IDENTITY.json` — **not** against `schema_version`, which upstream left at 30 while adding `display_identity` and `display_order` on 2026-09-19, the very change that caused the incident. Nothing is broken yet: a read that touches an unknown column may return an unusable response, and the symptom surfaces on the phone while the Mac looks healthy | 这台 Mac 的数据库比托管 Hermes 新，请升级托管副本。 | This Mac's database is newer than the managed Hermes. Update the managed copy. | No (update the managed Hermes) |
