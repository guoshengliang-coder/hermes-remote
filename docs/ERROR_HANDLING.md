...
| `HR-CRON-003` | An action on a scheduled job was refused or could not be sent: `POST /api/cron/jobs/{id}/{pause\|resume\|trigger}` returned a non-2xx the client cannot map to a stable code, or the request never completed. Distinct from `HR-CRON-002`, which is about a run that ALREADY happened — this one is about the tap the user just made. When the server does send a stable code, that code is shown instead of this one. **A timeout is not by itself this code**: 「立即运行」 runs the job synchronously upstream (`docs/HERMES_CONTRACT.md` §7c), so a job slower than the REST timeout times out while running. The client re-reads the job and, when `fire_claim` is held or `last_run_at` has moved, reports 「已触发，正在后台运行」 instead — this code is left for a timeout with no run behind it | 操作没有成功，请查看详情后重试。 | The action didn't go through. Review the details and retry. | Depends |
