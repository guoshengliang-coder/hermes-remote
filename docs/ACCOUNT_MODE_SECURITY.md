...
| A cold start looks like a sign-out | Users retype an email code several times a day, and get used to doing it — the worst habit an auth surface can teach | The app exchanges the refresh cookie once on boot before showing the login page (`GatewayClient.resume()`). It is the same CSRF-checked, rotating `POST /v2/web/auth/refresh` as any other refresh; nothing new is trusted, and a browser without a session still lands on a clean login page | Web app unit tests (`resume()` returns false silently); local drive with the access cookie removed |
| Shared or public computer keeps chat content | Later user reads conversations | Service worker caches only the app shell and session-list metadata, never chat content; sign-out or revocation clears every cache; access/refresh never in Web storage; `localStorage` holds the chosen Mac id, this browser's pinned session ids, unsent composer drafts, recent search queries and saved prompts (owner decision 2026-09-22), the default project's folder path and which bot-channel notices were seen — all removed on sign-out. Theme and language preferences also live in `localStorage`; they contain no chat or account data and remain after sign-out. | Web app unit tests; device smoke test |
| A session that outlives the device | A phone that is lost, sold or taken keeps working until someone notices and revokes it | The refresh window is 180 days and **rolling**, not unlimited: a client left unused that long loses access by itself, and every rotation invalidates its predecessor, so a copied token is caught as reuse and the whole family dies. "Never expires" was considered and refused for exactly this reason (owner decision 2026-09-23, raised from 30 days) | `account-mode.test.ts` pins the window and that a rotation restarts it |
