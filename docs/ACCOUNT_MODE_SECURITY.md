...
| Shared or public computer keeps chat content | Later user reads conversations | Service worker caches only the app shell and session-list metadata, never chat content; sign-out or revocation clears every cache; access/refresh never in Web storage; `localStorage` holds the chosen Mac id, this browser's pinned session ids and unsent composer drafts (owner decision 2026-09-22), all removed on sign-out | Web app unit tests; device smoke test |
