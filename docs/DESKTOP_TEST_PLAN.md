...
| Componentized v2 preflight card | Default package remains absent/inert; component runtime requires valid local v2 configuration plus Gateway schema 2 and `hermes-serve-v1`; a development package with complete signed-v2 configuration loads after account bootstrap and Refresh, then renders the verified card before the v1 installation card; absent/future server schema, runtime mismatch, failed trust, or failed scan stays hidden and non-actionable | Core capability decoding and fail-closed matrix, runtime, configuration, probe, presentation, and full SwiftUI compilation automated; enabled packaged UI run pending |
