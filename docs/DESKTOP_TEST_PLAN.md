...
| Managed patch 020 — remote reads drop inline image data (HG-74) | `session.resume` and `GET /api/sessions/{id}/messages` carry `[image]` in place of an inline `data:` image; the image still appears on the phone, from the Mac path in the text part; an `https://` image URL is untouched; the placeholder never reaches a bubble | Patch applies cleanly through `applyHermesPatches` onto the pinned tree and the result parses under the managed interpreter; both client-side behaviours the patch depends on are pinned by Android tests (`TimelineNoteTest`, `MultimodalContentMappingTest`); measured against the real database — one message 27,479,595→6,374 characters, the whole HG-65 session 105.07 MiB→0.265 MiB. **Not yet live**: needs a managed release and a controlled activation, so no phone has read a patched response |
