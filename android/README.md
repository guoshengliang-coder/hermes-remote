...
- Version 0.1.147 Android 0.1.147：冷启动恢复「已完成」结论前先核对未读标记，早已看过的旧结论不再反复出现（HG-138）。
- Version 0.1.146 Android 0.1.146：修复冷启动后会话列表重复显示「已完成」的问题（HG-137）。
- Version 0.1.145 改进 @url: 链接显示、应用图标及历史长内容加载。
- Version 0.1.144 修复新建会话与视频预览问题，改进模型切换体验及跨端诊断日志。
- Version 0.1.143 修复从会话列表进入会话不显示电脑端新消息、需手动刷新的问题；本地遗留的失败消息不再卡住自动同步（HG-124）
- Version 0.1.142 PC 端新消息自动同步到已打开的会话页；粘贴链接的会话不再把网页正文显示成你输入的内容（HG-124/HG-125）
- Version 0.1.141 Version 0.1.141
  会话列表与聊天记录改为分页加载并合并刷新，打开会话先显示最近消息、上滑加载更早内容（HG-104）；修复账号模式下本机记录缓存不生效。
- Version 0.1.140 FCM real-time push for account-mode phones with Google Play services (HG-94);
  streamed chat content stays continuous (HG-96).
- Version 0.1.139 修复 Relay/Connector 卡死请求的自动回收与离线状态恢复，并支持聊天正文原位选字复制。
...
- Version 0.1.134 跨端占用改为只读、运行态会主动校准，图片密集历史不再拖垮连接；并更新图库、加载与会话交互
- Version 0.1.133 历史加载失败按成因分类，不再一律显示同一个错误码
- Version 0.1.132 超大附件会话不再拖垮连接；PDF 改按文件发送
- Version 0.1.131 未读徽标不再凭空出现、带图会话历史能打开、停止按钮与新建会话失败都会说明原因，并为华为/荣耀补上桌面图标未读角标。
- Version 0.1.130 会话跑完后手机不再一直转圈，带图片的会话历史与下拉刷新不再同步不上
- Version 0.1.129 PDF 附件失败改说 Mac 缺渲染依赖并收回无效重试，会话运行状态不再漏判，长等待显示已运行时长
...
```text
app/build/outputs/apk/distribution/debug/Hermes-Remote-0.1.148-debug.apk
```
