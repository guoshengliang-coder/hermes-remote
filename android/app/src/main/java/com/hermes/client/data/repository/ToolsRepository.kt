...
class ToolsRepository(private val rest: HermesRestApi) {
    ...
    suspend fun createCron(
        prompt: String, schedule: String, name: String, deliver: String? = null, profile: String? = null,
    ) = rest.createCron(prompt, schedule, name, deliver, profile)
    suspend fun updateCron(
        jobId: String, prompt: String, schedule: String, name: String,
        deliver: String? = null, profile: String? = null,
    ) = rest.updateCron(jobId, prompt, schedule, name, deliver, profile)
    ...
    suspend fun restartGateway(profile: String? = null) = rest.restartGateway(profile)
    suspend fun cronDeliveryTargets() = rest.cronDeliveryTargets()
    suspend fun testMessagingPlatform(platformId: String, profile: String? = null) =
        rest.testMessagingPlatform(platformId, profile)
