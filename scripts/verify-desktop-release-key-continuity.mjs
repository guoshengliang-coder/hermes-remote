...
const [origin, keyId, publicKey] = process.argv.slice(2);
if (process.argv.length !== 5) {
  process.stderr.write("Usage: verify-desktop-release-key-continuity.mjs ORIGIN KEY_ID PUBLIC_KEY\n");
  process.exit(64);
}
try {
  const releases = await verifyDesktopReleaseKeyContinuity({ origin, keyId, publicKey });
